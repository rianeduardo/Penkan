...
if ($action === 'profile') {
    ...
    if ($new === '' || $current === '') { header('Location: account.php'); exit; }
    $st = DB::pdo()->prepare('SELECT password FROM users WHERE id = ? LIMIT 1');
    $st->execute([$uid]);
    $u = $st->fetch(PDO::FETCH_ASSOC);
    if (!$u || !password_verify($current, $u['password'])) { header('Location: account.php'); exit; }
